...
| `$ MMS` | Lower | Depends on unverified MMS syntax |

**Start with `@build_vms.com`.** It has far fewer ways to go wrong. If
`descrip.mms` fails, suspect the MMS syntax before suspecting the C.
...
- MMS comment and continuation syntax in `descrip.mms` is unverified.
- The OpenSSL shareable image name and pointer-size variant.
